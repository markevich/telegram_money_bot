defmodule MarkevichMoney.Release do
  ...
  def changelog("0.3.6") do
    ...
    *13 Апреля 2020*
    ...
    Технический релиз:
      *- шаг назад, два вперед. Все внутренние модули переписанны для упрощения их автоматического тестирования. А это значит что новый функционал будет появляться значительно чаще!*

    *Исправлено:*
      *- команда /add научилась распознавать траты состоящие из нескольких слов, например* `/add 10 Фантастическая курочка` *создаст транзакцию на* `10`* рублей и* `"Фантастическая курочка"`* в колонке* `"кому"`
    """
  end
